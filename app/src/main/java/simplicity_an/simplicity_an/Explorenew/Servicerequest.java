...
public Servicerequest(RequestInterface.Productlist requestProductlists){
    ...
    public Servicerequest(RequestInterface.Productdetail requestProductdetails){
        this.requestProductdetail=requestProductdetails;
    }
...
requestProductlist.Lowcategory(lowcatlist);
    ...
    public void getProductDetail(final String lang, final String rtype, final String profileid,final String maincatid,final String productid, Context context){
        vollRequestQueue=Volley.newRequestQueue(context);

        StringRequest company_request=new StringRequest(Request.Method.POST, Configurl.exploreurl, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Log.e("Response","shop__productdetail"+response.toString());
                ...
                try{
                    ...
                    if (product_count!=0) {
                        ...
                        for (int i = 0; i < product_array.length(); i++) {
                            ...
                            model.setPricelist(price);
                            if(maincatid.equals("2")){
                           JSONArray stonearray=obj.getJSONArray("ston_list");



                                List<IndexProductModel> stone = new ArrayList<>();
                                for (int j = 0; j < stonearray.length(); j++) {
                                    JSONObject obj_price = (JSONObject) stonearray.get(j);
                                    IndexProductModel model_price = new IndexProductModel();
                                    String name = obj_price.isNull("name") ? null : obj_price
                                            .getString("name");
                                    model.setStonename(name);
                                    model_price.setStonename(obj_price.getString("name"));
                                    model_price.setStonecut(obj_price.getString("cut"));
                                    model_price.setStonecolor(obj_price.getString("color"));
                                    model_price.setStoneclarity(obj_price.getString("clarity"));
                                    model_price.setStoneweight(obj_price.getString("weight"));
                                    model_price.setStoneprice(obj_price.getString("price"));

                                    stone.add(model_price);


                                }
                                model.setStonelist(stone);
                ...
                requestProductdetail.Productdetails(list);
